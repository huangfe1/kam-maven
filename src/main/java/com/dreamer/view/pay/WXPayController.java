	...
	public ResponseEntity<String> payNotify(@RequestBody String body,HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if(resData.getReturn_code().toUpperCase().equals("SUCCESS")){
				...
				//用
				BigDecimal b1 = new BigDecimal(Double.toString(order.getTotalMoney()));//该支付的金额
				BigDecimal b2 = new BigDecimal(Double.toString(resData.getCash_fee()*0.01));//实际支付的金额
				Double sub=b1.subtract(b2).doubleValue();//差额代金券负数
				order.setDiscountAmount(sub);
