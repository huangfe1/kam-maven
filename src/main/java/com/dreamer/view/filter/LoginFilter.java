...
public final class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest,
		...
		String requestPath = WebUtil.getRequstPathWithoutQueryString(request);

		if(WebUtil.isLogin(request)){//已经登录
            chain.doFilter(request, response);
            return;
        }else {//没有登录
            //微信登录
            if(requestPath.indexOf("/dmz/pmall/")>-1){
                ...
                if (ua.indexOf("micromessenger") > 0) {// 是微信浏览器
                    ...
                    String getCodeUrl = getOpenIdHandler.createGetUserInfoCodeCallbackUrl(payConfig,redirectUrl,state);
                    response.sendRedirect(getCodeUrl);
                    return;
                }
            }
            //不需要登录
            if(notNeedValidatePath(WebUtil.getRequstPathWithoutQueryString(request))){
                chain.doFilter(request, response);
                return;
            }
            String xhr = request.getHeader(XHR_HEADER_NAME);
            //访问手机页面
            if (requestPath.indexOf("/vmall/") > -1 || requestPath.indexOf("/pmall/")>-1||requestPath.indexOf("/tmall/")>-1){
                String redirectUrl = WebUtil.getContextPath(request) + VMALL_LOGIN_PATH ;
                if (Objects.equals(xhr, XHR_HEADER)) { //xml请求
                    response.setHeader("Location", redirectUrl+ "?url=" + URLEncoder.encode(request.getHeader("Referer"), "UTF-8"));
                ...
                String redirectUrl = WebUtil.getContextPath(request) + LOGIN_PATH + "?url=" + URLEncoder.encode(fullRequestPath, "UTF-8");
                if (Objects.equals(xhr, XHR_HEADER)) {//xml请求
