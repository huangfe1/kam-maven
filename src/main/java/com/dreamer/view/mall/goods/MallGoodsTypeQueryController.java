...
public class MallGoodsTypeQueryController {

	@RequestMapping(value = { "/index.html", "/search.html" }, method = RequestMethod.GET)
	public String index(
			@ModelAttribute("parameter") SearchParameter<MallGoodsType> parameter,
			HttpServletRequest request, Model model) {
		try {
			List<MallGoodsType> goods = mallGoodsTypeDAO.searchEntityByPage(parameter, null,
					(t) -> true);
	...
	public SearchParameter<MallGoodsType> preprocess(
			@RequestParam("id") Optional<Integer> id,@RequestParam("parentType") Optional<Integer> parentType) {
		SearchParameter<MallGoodsType> parameter = new SearchParameter<>();
