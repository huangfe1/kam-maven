...
//		}
	...
	public void pay(PaymentWay paymentWay){
		...
		BigDecimal b1 = new BigDecimal(Double.toString(getTotalPoints()));
		if(Objects.isNull(getDiscountAmount())){//如果是0的话
				setDiscountAmount(0.0);
		}
		BigDecimal b2 = new BigDecimal(Double.toString(getDiscountAmount()));//负数
//			BigDecimal b3 = new BigDecimal(Double.toString(order.getActualPayment()));
		Double sub=b1.add(b2).doubleValue();
		//代金券要少扣，因为这部分这算成现金了 getDiscountAmount是负数
        user.getAccounts().deductAdvance(sub,moreStr.toString());
	}
