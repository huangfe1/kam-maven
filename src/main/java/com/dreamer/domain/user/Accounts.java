...
public class Accounts implements java.io.Serializable {
	...
	private Double logisticsFeeBalance;//物流费
...
//		acs.increaseVoucher(voucher,more);
	 ...
	 * @param more
	 */
	public void transferLogistFeeToAnother(Accounts acs,Double fee){
		String more=user.getRealName()+"主动转移物流费到"+acs.getUser().getRealName();
		if(!user.isMutedUser()){
			this.deductLogistFee(fee,more);//本人减少
		}
		acs.increaseLogistFee(fee,more);//接收方增加
	}
...
//		}
	 ...
	 * @return
	 */
	public Double increaseLogistFee(Double logistFee,String more){
		if(logistFee==0)return  getLogisticsFeeBalance();//不处理
		if(logistFee<0){
			throw new ApplicationException("物流费增加值非法");
		}
		// 进行加法运算
		BigDecimal b1 = new BigDecimal(getLogisticsFeeBalance());
		BigDecimal b2 = new BigDecimal(logistFee);
		setLogisticsFeeBalance(b1.add(b2).doubleValue());
		user.addLogistFeeRecord(1,more,b2.doubleValue());
		return getLogisticsFeeBalance();
	}
	 ...
	 * @return
	 */
	public Double deductLogistFee(Double logistFee,String more){
		if(logistFee==0)return  getAdvanceBalance();//不处理

		if(logistFee<0){
			throw new ApplicationException("物流费扣减值不能为负数");
		}
		if(logistFee>getLogisticsFeeBalance()){
			throw new ApplicationException("物流费余额不足");
		}
		// 进行减法运算
		BigDecimal b1 = new BigDecimal(getLogisticsFeeBalance());
		BigDecimal b2 = new BigDecimal(logistFee);
		setLogisticsFeeBalance(b1.subtract(b2).doubleValue());
		user.addLogistFeeRecord(0,more,b2.doubleValue());
		return  getLogisticsFeeBalance();
	}
    ...
    public Double getLogisticsFeeBalance() {
        return logisticsFeeBalance;
    }

    public void setLogisticsFeeBalance(Double logisticsFeeBalance) {
        this.logisticsFeeBalance = logisticsFeeBalance;
    }
