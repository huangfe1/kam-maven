...
public class MallGoodsDAO extends HibernateBaseDAO<MallGoods>{
	...
	public List<MallGoods> searchEntityByPage(SearchParameter<MallGoods> p,
			...
			Function<Void, Boolean> getCacheQueries,Integer mallType) {
		...
		return super.searchEntityByPage(p, (t)->{
			...
			if(t.getEntity().getMallGoodsType()!=null&&t.getEntity().getMallGoodsType().getId()!=null){
                criteria.add(Restrictions.eq("mallGoodsType.id",t.getEntity().getMallGoodsType().getId()));
            }
            if(mallType!=null&&mallType==0){//优惠商城
					criteria.add(Restrictions.ne("moneyPrice",0.0));
			}else {
				criteria.add(Restrictions.eq("moneyPrice",0.0));
			}
			criteria.add(Restrictions.ne("stockQuantity",0));//库存不为0
     ...
     * @return
     */
	public List<MallGoods> findByType(Integer type) {
        try {
            String queryString = "from MallGoods as model where model.mallGoodsType.id = ? and model.shelf = true and model.stockQuantity>0 order by model.sequence";
            Query queryObject = getCurrentSession().createQuery(queryString);
            queryObject.setParameter(0, type);
            return queryObject.list();
        }catch (Exception e){
            log.error("find by type id name failed", e);
            throw e;
        }
    }
	...
	public List<MallGoods> findAll(int start,int max) {
		log.debug("finding all MallGoods instances");
		try {
			String queryString = "from MallGoods where shelf = true ";
			Query queryObject = getCurrentSession().createQuery(queryString);
			queryObject.setFirstResult(start).setMaxResults(max);
			return queryObject.list();
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
