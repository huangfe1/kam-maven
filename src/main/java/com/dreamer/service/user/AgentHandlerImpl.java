...
//		Long count=agentDAO.countAgent(agent.getMobile(), agent.getWeixin());
	 ...
	 * @param agent
	 */
	public void georganize(Agent agent){
		List<Agent> childres = agentDAO.findByParent(agent);
		if(childres!=null&&!childres.isEmpty()){
            for(Agent children : childres){
				children.reorganizeAll();//取消所有授权
				georganize(children);
			}
		}
	}
	...
	@Override
	@Transactional
	public void georganizeChildrens(User operator, Agent agent) {
        agent.reorganizeAll();//先取消自己的
		georganize(agent);//逐级取消下级授权
		agentDAO.merge(agent);
	}
	...
	@Transactional
	@Override
	public void transferLogistFee(User operator, LogistFeeTransfer transfer,
								String toAgentCode, String toAgentName, Double fee) {
		// TODO Auto-generated method stub
		Agent toAgent;
		if(toAgentCode.equals("01")){
			toAgent=mutedUserDAO.loadFirstOne();
		...
		transfer.transferfee(fromAgent, toAgent, fee);
		logistFeeTransferDAO.merge(transfer);
	}
	...
	@Override
	@Transactional
	public void addLogistFee(User operator, LogistFeeTransfer transfer) {
		Agent user_temp = mutedUserDAO.findById(3);//公司
		transfer.commit(user_temp,operator);//提交订单
		logistFeeTransferDAO.save(transfer);//保存
	}

	@Transactional
	@Override
	public Agent bulidVisitor(String openId, String nickName, String headImg) {
		List<Agent> agents = agentDAO.findByProperty("wxOpenid",openId);
		Agent agent;
...
//			agent.setHeadimgurl(headImg);
	...
	@Override
	@Transactional
	public void payForLogistFee(String time, LogistFeeTransfer transfer) {
		transfer.payForfee(time);
		logistFeeTransferDAO.merge(transfer);//更新transfer
	}
