							...
							<tbody id="dataList">
								<c:forEach items="${agents}" var="l">
									<tr data-row="${l.id}">
										...
										<td>${l.accounts.logisticsFeeBalance}</td>
										...
										<a class="btn btn-info editBtn" href="<c:url value='/agent/accounts_detail.html?id=${l.id}' /> ">账户明细</a>
											...
											<c:if test="${l.active}">
											<a data-loading-text="正在整顿...." class="btn btn-warning default ajaxLink" data-role="reorganize"
											href="<c:url value='/agent/reorganize.json?id=${l.id}&&type=0' /> "><span class="fa fa-lock fa-fw" aria-hidden="true"></span>单个整顿</a>
                                                <a data-loading-text="正在整顿...." class="btn btn-warning default ajaxLink" data-role="reorganize"
                                                   href="<c:url value='/agent/reorganize.json?id=${l.id}&&type=1' /> "><span class="fa fa-lock fa-fw" aria-hidden="true"></span>全部整顿</a>
                                            </c:if>
