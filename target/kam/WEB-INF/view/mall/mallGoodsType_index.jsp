					...
					<div class="collapse navbar-collapse" id="top-navbar-collapse">
						<%--<form class="navbar-form navbar-left" id="searchForm"--%>
							...
							<%--<div class="form-group">--%>
								...
								<%--&lt;%&ndash;<label class="">积分系数</label> <input type="number"&ndash;%&gt;--%>
									<%--&lt;%&ndash;value="${parameter.entity.pointFactor}" name="entity.pointFactor" id="pointFactor"&ndash;%&gt;--%>
									<%--&lt;%&ndash;autofocus class="typeahead form-control" placeholder="产品积分系数">&ndash;%&gt;--%>
						...
						<table
							...
							<tbody id="dataList">
								<c:forEach items="${goodsType}" var="g">
									<tr  data-row="${g.id}">
										<td style="padding-left:${g.type*10}px;"><a class="editBtn"
											href="<c:url value='/mall/goods/type/edit.html?id=${g.id}' /> ">${g.name}</a></td>
										<td>${g.orderIndex}</td>
										<td>${g.updateTime}</td>
										<td><a class="btn btn-success default editBtn"
											href="<c:url value='/mall/goods/type/edit.html?id=${g.id}' /> "><span class="glyphicon glyphicon-wrench" aria-hidden="true"></span>修改</a>
											<a class="ajaxLink btn btn-danger default" data-role="delete"
											href="<c:url value='/mall/goods/type/remove.json?id=${g.id}' /> "><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>删除</a>
											<a class="btn btn-success default editBtn"
											   href="<c:url value='/mall/goods/type/edit.html?parentType=${g.id}' /> "><span class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增子等级</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
		...
		$(function() {
			$(".addBtn").click(
					function(e) {
						...
						e.stopPropagation();
						$("#myModal").load(
								"<c:url value='/mall/goods/type/edit.html'/>",
								function(e) {
									$('#myModal').modal({
										backdrop : "static"
									});
